...
class Servo : public nodelet::Nodelet {
...
private:
  ...
  ros::ServiceServer servo_service_server_;
  ...
  bool callbackServo(pairs_msgs::SetIntRequest &req, pairs_msgs::SetIntResponse &res);
...
void Servo::onInit() {
  ...
  baca_protocol_publisher_ = nh_.advertise<pairs_modules_msgs::BacaProtocol>("baca_protocol_out", 1);

  baca_protocol_subscriber = nh_.subscribe("baca_protocol_in", 10, &Servo::callbackSendMessage, this, ros::TransportHints().tcpNoDelay());

  raw_message_subscriber = nh_.subscribe("raw_in", 10, &Servo::callbackSendRawMessage, this, ros::TransportHints().tcpNoDelay());

  servo_service_server_ = nh_.advertiseService("move", &Servo::callbackServo, this);
  ...
  serial_timer_     = nh_.createTimer(ros::Rate(serial_rate_), &Servo::callbackSerialTimer, this);
  fake_timer_       = nh_.createTimer(ros::Rate(fake_garmin_rate_), &Servo::callbackFakeTimer, this);
  maintainer_timer_ = nh_.createTimer(ros::Rate(1), &Servo::callbackMaintainerTimer, this);
...
void Servo::callbackSerialTimer(const ros::TimerEvent &event) {
...
void Servo::callbackFakeTimer(const ros::TimerEvent &event) {
...
void Servo::callbackMaintainerTimer(const ros::TimerEvent &event) {
...
void Servo::callbackSendMessage(const pairs_modules_msgs::BacaProtocolConstPtr &msg) {
...
void Servo::callbackSendRawMessage(const pairs_modules_msgs::SerialRawConstPtr &msg) {
...
bool Servo::callbackServo([[maybe_unused]] pairs_msgs::SetIntRequest &req, pairs_msgs::SetIntResponse &res) {

  int desired_pos = req.value;

  if (desired_pos > 1024) {

    res.success = false;
    res.message = "value out of range, max 1023";
  ...
  payl.push_back(0x86);
  payl.push_back((desired_pos >> 8) & 0xFF);
  payl.push_back(desired_pos & 0xFF);
...
void Servo::interpretSerialData(uint8_t single_character) {
...
void Servo::processMessage(uint8_t payload_size, uint8_t *input_buffer, uint8_t checksum, uint8_t checksum_rec, bool checksum_correct) {
  ...
  if (payload_size == 3 && (input_buffer[0] == 0x00 || input_buffer[0] == 0x01) && checksum_correct) {
    ...
    try {
      ...
      ROS_ERROR("[PairsSerial]: exception caught during publishing topic %s", baca_protocol_publisher_.getTopic().c_str());
    }
  }
}
...
uint8_t Servo::connectToSensor(void) {
