...
namespace gimbal
{
    ...
    void Gimbal::onInit()
    {
      ...
      if (connected)
      {
        ...
        m_pub_orientation_pry = m_nh.advertise<pairs_modules_msgs::GimbalPRY>("attitude_out_pry", 10);
        ...
        m_transformer = std::make_unique<pairs_lib::Transformer>("Gimbal");
...
//}
    ...
    /* cmd_pry_cbk() method //{ */
    void Gimbal::cmd_pry_cbk(const pairs_modules_msgs::GimbalPRY::ConstPtr &cmd_pry) {
    ...
    void Gimbal::process_custom_data_msg(const SBGC_cmd_realtime_data_custom_t &data) {
        ...
        if (m_request_data_flags & cmd_realtime_data_custom_flags_stator_rotor_angle) {
            auto msg_pry = boost::make_shared<pairs_modules_msgs::GimbalPRY>();
