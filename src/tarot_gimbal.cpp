...
#include <pairs_msgs/GimbalState.h>
...
private:
  ...
  void callbackSendCommand(const pairs_msgs::GimbalState &msg);
...
void TarotGimbal::onInit() {
  ...
  gimbal_status_publisher  = nh_.advertise<pairs_msgs::GimbalState>("gimbal_state", 1);
...
void TarotGimbal::callbackSendRawMessage(const pairs_modules_msgs::SerialRawConstPtr &msg) {
...
void TarotGimbal::callbackSendCommand(const pairs_msgs::GimbalState &msg) {
...
void TarotGimbal::processMessage(uint8_t payload_size, uint8_t *input_buffer, uint8_t checksum, uint8_t checksum_rec, bool checksum_correct) {
  ...
  if (payload_size == 6 && (input_buffer[0] == 0x1A) && checksum_correct) {
    ...
    pairs_msgs::GimbalState gimbal_msg;
    ...
    try {
      ...
      ROS_ERROR("[PairsSerial]: exception caught during publishing topic %s", gimbal_status_publisher.getTopic().c_str());
    }
