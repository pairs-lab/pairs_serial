...
#include <string>
#include <pairs_modules_msgs/BacaProtocol.h>
#include <pairs_modules_msgs/SerialRaw.h>
...
class Eagle : public nodelet::Nodelet {
...
private:
  ...
  ros::ServiceServer ser_net_open;
  ros::ServiceServer ser_net_close;
  ros::ServiceServer ser_net_drop;
  ros::ServiceServer ser_net_undrop;
  ...
  void callbackSendMessage(const pairs_modules_msgs::BacaProtocolConstPtr &msg);
  void callbackSendRawMessage(const pairs_modules_msgs::SerialRawConstPtr &msg);
  void callbackMagnet(const std_msgs::EmptyConstPtr &msg);

  bool callbackSendInt([[maybe_unused]] pairs_msgs::SetInt::Request &req, pairs_msgs::SetInt::Response &res);
  bool callbackSendIntRaw([[maybe_unused]] pairs_msgs::SetInt::Request &req, pairs_msgs::SetInt::Response &res);

  bool callbackNetOpen([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
  bool callbackNetClose([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
  bool callbackNetDrop([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
  bool callbackNetUndrop([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
  ...
  ros::Publisher eagle_publisher_;

  ros::Subscriber raw_message_subscriber;
  ros::Subscriber eagle_subscriber;
...
void Eagle::onInit() {
  ...
  ser_send_int     = nh_.advertiseService("send_int", &Eagle::callbackSendInt, this);
  ser_send_int_raw = nh_.advertiseService("send_int_raw", &Eagle::callbackSendIntRaw, this);

  ser_net_open = nh_.advertiseService("net_open", &Eagle::callbackNetOpen, this);
  ser_net_close = nh_.advertiseService("net_close", &Eagle::callbackNetClose, this);
  ser_net_drop = nh_.advertiseService("net_drop", &Eagle::callbackNetDrop, this);
  ser_net_undrop = nh_.advertiseService("net_undrop", &Eagle::callbackNetUndrop, this);
  ...
  eagle_publisher_ = nh_.advertise<pairs_modules_msgs::BacaProtocol>("eagle_out", 1);

  eagle_subscriber = nh_.subscribe("eagle_in", 10, &Eagle::callbackSendMessage, this, ros::TransportHints().tcpNoDelay());

  raw_message_subscriber = nh_.subscribe("raw_in", 10, &Eagle::callbackSendRawMessage, this, ros::TransportHints().tcpNoDelay());
  ...
  serial_timer_     = nh_.createTimer(ros::Rate(serial_rate_), &Eagle::callbackSerialTimer, this);
  fake_timer_       = nh_.createTimer(ros::Rate(fake_garmin_rate_), &Eagle::callbackFakeTimer, this);
  maintainer_timer_ = nh_.createTimer(ros::Rate(1), &Eagle::callbackMaintainerTimer, this);
...
void Eagle::callbackSerialTimer(const ros::TimerEvent &event) {
...
void Eagle::callbackFakeTimer(const ros::TimerEvent &event) {
...
void Eagle::callbackMaintainerTimer(const ros::TimerEvent &event) {
...
void Eagle::callbackSendMessage(const pairs_modules_msgs::BacaProtocolConstPtr &msg) {
...
void Eagle::callbackSendRawMessage(const pairs_modules_msgs::SerialRawConstPtr &msg) {
...
bool Eagle::callbackSendInt([[maybe_unused]] pairs_msgs::SetInt::Request &req, pairs_msgs::SetInt::Response &res) {
  ...
  ROS_WARN_STREAM("[Eagle]: Sending: 0x" << hex_string);
...
bool Eagle::callbackSendIntRaw([[maybe_unused]] pairs_msgs::SetInt::Request &req, pairs_msgs::SetInt::Response &res) {
...
bool Eagle::callbackNetOpen([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res) {
...
bool Eagle::callbackNetClose([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res) {

  if (!is_initialized_) {
    return false;
  }

  uint8_t payload      = uint8_t(1);
  uint8_t payload_size = 1;
  uint8_t out_buffer[payload_size];
  out_buffer[0] = payload;
...
bool Eagle::callbackNetDrop([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res) {

  if (!is_initialized_) {
    return false;
  }

  uint8_t payload      = uint8_t(2);
  uint8_t payload_size = 1;
  uint8_t out_buffer[payload_size];
  out_buffer[0] = payload;
...
bool Eagle::callbackNetUndrop([[maybe_unused]] std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res) {

  if (!is_initialized_) {
    return false;
  }

  uint8_t payload      = uint8_t(3);
  uint8_t payload_size = 1;
  uint8_t out_buffer[payload_size];
  out_buffer[0] = payload;
...
void Eagle::interpretSerialData(uint8_t single_character) {
...
void Eagle::processMessage(uint8_t payload_size, uint8_t *input_buffer, uint8_t checksum, uint8_t checksum_rec, bool checksum_correct) {
  ...
  if (payload_size == 3 && (input_buffer[0] == 0x00 || input_buffer[0] == 0x01) && checksum_correct) {
    ...
    if (message_id == 0x00) {
      ...
      try {
        ...
        ROS_ERROR("[PairsSerial]: exception caught during publishing topic %s", range_publisher_A_.getTopic().c_str());
      }
      ...
      try {
        ...
        ROS_ERROR("[PairsSerial]: exception caught during publishing topic %s", range_publisher_B_.getTopic().c_str());
      }
    }
  } else {
    ...
    pairs_modules_msgs::BacaProtocol msg;
    ...
    try {
      eagle_publisher_.publish(msg);
    }
    catch (...) {
      ROS_ERROR("[PairsSerial]: exception caught during publishing topic %s", eagle_publisher_.getTopic().c_str());
    }
  }
}
...
uint8_t Eagle::connectToSensor(void) {
...
PLUGINLIB_EXPORT_CLASS(eagle::Eagle, nodelet::Nodelet);
